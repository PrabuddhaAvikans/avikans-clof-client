...
export type EstimationListPanelProps = {
...
export function EstimationListPanel({
  items,
  totalCount,
  selectedId,
  onSelect,
  ...
  return (
    <div className={cn(workspaceListPanelShell, className)}>
      <div className="border-b border-border px-3 py-3">
        <h2 className="text-sm font-semibold text-foreground">Estimation Requests</h2>
        <p className="mt-0.5 text-xs text-muted-foreground">
          {totalCount} estimation request{totalCount !== 1 ? "s" : ""}
        </p>
      ...
      <div className={workspaceListPanelBody}>
        {isLoading ? (
          ...
          <p className="p-6 text-center text-sm text-muted-foreground">
            No estimation requests match your filters.
          </p>
        ) : (
